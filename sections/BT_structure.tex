...
\label{sec:Crossing-BT}
    ...
    With condition node \texttt{CollisionBwdMove} we check if the robot can move backward. If it can, we do so, providing there would be a collision on stop. If there would be a collision, we will stop the robot instead.\\
