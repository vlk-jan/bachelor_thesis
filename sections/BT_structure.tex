\section{Creating a behavior tree structure}
    ...
    The second approach is creating an initial BT and letting RL algorithms improve its functionality and optimality. There are several options for this particular approach.\\
    ...
    Each of the presented approaches have their advantages and disadvantages. It is therefore vital to select the correct approach based on the possibilities and requirements of the task.\\\\
    \bfc{Chosen approach}\\
        ...
        Using different approaches to designing and improving the BT structure may be an interesting task for future works.\\
...
\label{sec:Crossing-BT}
    This tree is the most important part of the whole algorithm as it facilitates the crossing of the road. Figure \ref{fig:Crossing-BT} shows the structure of the tree.\\
    ...
    The second branch starts with the \texttt{Fallback} node. The first child of this node is a condition node \texttt{CarsInTrajectory}. This node checks if there are any cars in the trajectory of the robot. If there are not, we are free to continue with our movement or start it if we have not done so yet. This is performed with a sub-tree \texttt{StartCrossing}, followed by the \texttt{ContinueMovement} action node. These nodes are connected behind a \texttt{Sequence} node.\\
    The following two branches are responsible for crossing the road if cars were detected in the robot's trajectory. The third node is the \texttt{StartCrossing} sub-tree.\\
    The fourth branch is the decision making part of the tree. It starts with the \texttt{Sequence} node with its first child the \texttt{CalculateCollision} action node. This node is responsible for calculating the velocities for the robot to collide with all detected vehicles. These velocities are a vital information on which all the decision making is based.\\
    The second child of the \texttt{Sequence} node is a structure of cascading if-else statements. The structure gradually checks the following conditions and based on the results perfors the corresponding actions.\\
    The first condition is the \texttt{CollisionImminent} condition node. This node checks if the robot is about to collide with any of the detected vehicles. If it is no, the robot will continue its current movement.\\
    It there is a collision detected we check if the collision would be on movement forward with a \texttt{CollisionFwdMove}. If not we can continue with forward movement, however the speed could change.\\
    If the collision would be on forward movement, we check if the robot can move backwards with a \texttt{CollisionBwdMove} condition node. If it can, we move backwards with the \texttt{MoveBwd} node. If not, we use the \texttt{StopMovement} to stop the robot.\\
    \begin{figure}[H]
        \begin{tikzpicture}[sibling distance=28mm, minimum width=1cm, minimum height=0.8cm]
            \node [draw] {$Root$}
                child {node [draw] {$\to$} edge from parent [-{Stealth[length=2.5mm]}]
                child {node [draw, xshift=0-1.2cm] {GetCars}}
                child {node [draw, xshift=0-1.5cm] {?}
                child {node [ellipse, draw, xshift=0.3cm] {CarsInTrajectory}}
                child {node [draw, xshift=0.4cm] {$\to$}
                child {node [diamond, draw, aspect=2.3, xshift=0-1.8cm, yshift=0-1.5cm] {StartCrossing}}
                child {node [draw, xshift=0-0.75cm, yshift=0-1.5cm] {ContinueMovement}}}}
                child {node [diamond, draw, aspect=2.3, xshift=0-1.3cm] {StartCrossing}}
                child {node [draw, xshift=0-1.4cm] {$\to$}
                child {node [draw, xshift=0.4cm, yshift=0-1.5cm] {CalculateCollision}}
                child {node [draw, yshift=0-1.5cm] {?}
                child {node [draw, xshift=0-0.8cm] {$\to$}
                child {node [ellipse, draw, xshift=0-0.3cm] {CollisionImminent}}
                child {node [draw, xshift=0] {?}
                child {node [draw] {$\to$}
                child {node [ellipse, draw, xshift=0-0.2cm] {CollisionFwdMove}}
                child {node [draw] {?}
                child {node [draw] {$\to$}
                child {node [ellipse, draw, xshift=0-1.2cm] {CollisionBwdMove}}
                child {node [draw] {StopMovement}}}
                child {node [draw, xshift=0-0.7cm] {MoveBwd}}}}
                child {node [draw, xshift=0-0.9cm] {MoveFwd}}}}
                child {node [draw, xshift=0-1cm] {ContinueMovement}}}}
                child {node [diamond, draw, aspect=2.3, xshift=0-2cm] {Finished}}};
        \end{tikzpicture}
