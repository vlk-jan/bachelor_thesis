...
\cs_new:Nx \ctuthesis_geometry_hmarginratio: {6\string :4}

% Target textwidth is 33*basesize (10pt to 12pt)
\dim_const:Nn \g_ctuthesis_geometry_textwidth_dim { \tl_use:N \g_ctuthesis_fontsize_tl * 35 }
...
\dim_const:Nn \g_ctuthesis_geometry_innermargin_dim_front_pages { 1in + \the\evensidemargin }

% The frontmatter pages are a bit wider
\cs_new_protected:Nn \ctuthesis_geometry_title: {
	\newgeometry {
		% Use the stored innermargin value
		inner = \g_ctuthesis_geometry_innermargin_dim_front_pages ,
...
\ctuifswitch { pkg-hyperref } {
	...
	\tl_set:Nx \g_ctuthesis_hyperref_pdfinfo_tl {
		pdfinfo = {
			Title = \ctuthesis_field_use:nn { Bachelor's\ thesis } {\ Vlk\ Jan },
			Author = \ctuthesis_field_use:n { Vlk\ Jan },
			Subject = \ctuthesis_field_use:n { Autonomous\ Road\ Crossing\ with\ a\ Mobile\ Robot },
		}
	} 
